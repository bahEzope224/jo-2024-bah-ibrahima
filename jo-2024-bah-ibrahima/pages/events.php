...
<body>
    ...
    <main>
        <h1>Calendrier des epreuves</h1>
        <?php
        require_once("../database/database.php");

        try {
            // Requête pour récupérer la liste des sports depuis la base de données
            $query = "SELECT nom_epreuve, 
            DATE_FORMAT(date_epreuve,'%d/%m/%Y') AS date_epreuve_format,
            DATE_FORMAT(heure_epreuve,'%H:%i') AS horaire_epreuve, 
            nom_lieu, ville_lieu
            From epreuve
            ...
            if ($statement->rowCount() > 0) {
                echo "<table>";
                echo "<tr><th class='color'>Epreuves</th>
               ...
               <th class='color'>Ville</th>
                ...
                while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    echo "<td>" . htmlspecialchars($row['horaire_epreuve']) . "</td>";
